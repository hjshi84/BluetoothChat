...
public class GateDetect {
    ...
    private static class fallState{
        ...
        private Byte isFall(){
            ...
            lastValue=nowValue;
            if(endfall){
                if(endValue.timestamp-startValue.timestamp<85&&midValue.timestamp-startValue.timestamp<50){ //65,35
                    startfall=false;
                    midfall=false;
                    endfall=false;
                    return 0x01;
    ...
    private static class gateState{
        ...
        RecData lastValue=new RecData();
        float dynamicthreshold=0.98f;
        float precise=0.08f;
        int lastGateTime;
        int cycleTime=0;
        float max=-65535,min=65535;
        private Byte isGate(RecData temp){
            cycleTime++;
            if(cycleTime<200)
            {
                max=temp.accRecData.getSumValue()>max? temp.accRecData.getSumValue():max;
                min=temp.accRecData.getSumValue()<min? temp.accRecData.getSumValue():min;
            }else
            {
                dynamicthreshold=(max-min)/2;
                cycleTime=0;
            }
            if(sample_new.timestamp-lastGateTime>=3000)                                            //2000
            {
                lastGateTime=-100;
            }
            ...
            if(prejudge(temp)==1&&sample_old!=null)
            {
                if(sample_old.accRecData.getSumValue()>dynamicthreshold
                        ...
                        &&Math.abs(sample_new.timestamp-lastGateTime)>30)               //20
                {
